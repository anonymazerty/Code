...
PROMPT_TYPES = ["calibrated", "role-anchored", "task-constrained"]
...
def build_merged_table(crowd_df: pd.DataFrame, model_dfs: dict) -> pd.DataFrame:
    """
    ...
    rows = []
    abs_delta_cols = {
        f"{model_label[m]}_{lbl}": []
        for m in MODELS
        for _, lbl in EDU_GROUPS
    }
...
def main():
    for prompt in PROMPT_TYPES:
        ...
        crowd_df = pd.read_csv(os.path.join(prompt_dir, "crowd_survey.csv"))

        model_dfs = {}
        for model in MODELS:
            med_path = os.path.join(prompt_dir, model, "med_survey.csv")
            llm_df = pd.read_csv(med_path)
            model_dfs[model] = llm_df

            table = build_delta_table(crowd_df, llm_df)
            out_path = os.path.join(prompt_dir, model, "delta_by_edu.csv")
            table.to_csv(out_path, index=False)
            print(f"Saved: {out_path}")
            print(table.to_string(index=False))
            print()

        # Merged table for this prompt type
        merged = build_merged_table(crowd_df, model_dfs)
        merged_path = os.path.join(prompt_dir, "delta_by_edu_all_models.csv")
