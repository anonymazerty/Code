"""
Prompt templates for reflection-based orchestrator.

This module contains the prompt templates used by the reflection-based orchestrator
for deciding whether to simulate a policy rollout or make a direct decision.
"""
...
class ReflectionBasedPromptTemplates:
    """Collection of prompt templates for reflection-based orchestrator."""
    ...
    # System prompt for reflection-based orchestrator
    SYSTEM_PROMPT = PromptTemplate(
        input_variables=["target_skill_bundle", "objectives", "format_instructions", "long_term_memory", "max_rollouts", "rollout_steps"],
...
## Guardrails
- Always select exactly one policy as either a tool call to get student feedback after {rollout_steps}-step rollout or final decision.
...
- You have at maximum {max_rollouts} tool calls to get student feedback of the tools.
...
## Instructions
- Base your decision on the provided context (student state, available tools, rollout history, recent interactions, and long-term summary of past interactions).
...
"""
    ...
    FEW_SHOT_POLICY_SELECTION = PromptTemplate(
        ...
        template="""
## Query
Select one best policy to use for recommending the next set of questions to the student, based on the current student state, the available tools, and the tool call rollout history.
...
## Rollout History (Most recent last)
{rollouts_history}
