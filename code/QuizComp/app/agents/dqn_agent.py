    ...
    def load(self, path):
        try:
            ...
            if "size mismatch" in str(e):
                # Extract dimension information from error
                raise RuntimeError(
                    f"Model dimension mismatch when loading checkpoint from {path}. "
                    f"The saved model expects a different state_dim than the current environment. "
                    f"Original error: {str(e)}\n\n"
                    f"This likely means the universe was generated with different parameters "
                    f"(numTopics or numDifficulties) than the model was trained with. "
                    f"Please ensure the universe matches the training configuration."
                ) from e
            else:
                raise 